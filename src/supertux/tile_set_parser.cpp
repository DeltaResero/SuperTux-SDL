...
TileSetParser::parse_tile(const Reader& reader)
{
  ...
  std::unique_ptr<Tile> tile(new Tile(m_tileset, imagespecs, editor_imagespecs, attributes, data, fps));
  ...
  else
  {
    for(std::vector<uint32_t>::size_type i = 0; i < ids.size() && i < width*height; ++i) 
    {
      if (ids[i] != 0)
      {
        ...
        std::unique_ptr<Tile> tile(new Tile(m_tileset, imagespecs, editor_imagespecs,
